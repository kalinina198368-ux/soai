...
func (s QinNiuOss) PutUrlFile(fileURL string, useProxy bool) (string, error) {
	...
	if err != nil {
		log.Errorf("qiniu put url file download failed url=%s useProxy=%v err=%v elapsed=%s", fileURL, useProxy, err, time.Since(downloadStart))
		return "", fmt.Errorf("error with download image: %v", err)
	}
	log.Infof("qiniu put url file downloaded url=%s useProxy=%v size=%d elapsed=%s", fileURL, useProxy, len(fileData), time.Since(downloadStart))

	parse, err := url.Parse(fileURL)
	if err != nil {
		log.Errorf("qiniu put url file parse failed url=%s err=%v", fileURL, err)
	...
	if err != nil {
		log.Errorf("qiniu put url file upload failed url=%s key=%s err=%v elapsed=%s total=%s", fileURL, key, err, time.Since(uploadStart), time.Since(totalStart))
		return "", err
	}
	log.Infof("qiniu put url file uploaded url=%s key=%s elapsed=%s total=%s", fileURL, key, time.Since(uploadStart), time.Since(totalStart))
