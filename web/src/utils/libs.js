...
export function processContent(content) {
    ...
    // 先找到所有已经在markdown中的图片URL
    const markdownImageRegex = /!?\[.*?\]\((https?:\/\/[^\s)]+\.(jpg|jpeg|png|gif|bmp|svg))\)/gi;
    const markdownImages = [];
    let match;
    
    // 收集所有已经在markdown中的图片
    while ((match = markdownImageRegex.exec(content)) !== null) {
        markdownImages.push(match[0]); // 整个markdown语法
    }
    
    // 创建一个临时内容，将已存在的markdown图片替换为占位符
    let tempContent = content;
    const placeholders = [];
    markdownImages.forEach((markdownImg, index) => {
        const placeholder = `__MARKDOWN_IMG_${index}__`;
        placeholders.push(markdownImg);
        const escapedMarkdown = markdownImg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        tempContent = tempContent.replace(new RegExp(escapedMarkdown, 'g'), placeholder);
    });
    
    // 现在处理纯文本中的图片URL
    const urlRegex = /https?:\/\/[^\s<>)]+\.(jpg|jpeg|png|gif|bmp|svg)/gi;
    ...
    // 恢复markdown图片
    placeholders.forEach((markdownImg, index) => {
        processedContent = processedContent.replace(`__MARKDOWN_IMG_${index}__`, markdownImg);
    });
