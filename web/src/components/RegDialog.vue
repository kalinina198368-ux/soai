<template>
  <div class="register-dialog w-full">
    <div class="register-box w-full">
      <el-form :model="data" class="form" v-if="enableRegister">
        ...
        <div class="block">
          <el-input placeholder="邀请码(可选)" size="large" v-model="data.invite_code" autocomplete="off" :disabled="isInviteCodeFromQuery">
            <template #prefix>
              <el-icon>
                <Message />
              </el-icon>
...
// 判断邀请码是否来自URL参数
const isInviteCodeFromQuery = computed(() => {
  const inviteFromQuery = Array.isArray(route.query["invite_code"])
    ? route.query["invite_code"][0]
    : route.query["invite_code"];
  return typeof inviteFromQuery === "string" && inviteFromQuery !== "";
});

const recordInviteHit = (code) => {
  if (!code) {
    return;
  }
  httpGet("/api/invite/hits", { code }).catch(() => {});
};
...
onMounted(() => {
  const inviteFromQuery = Array.isArray(route.query["invite_code"])
    ? route.query["invite_code"][0]
    : route.query["invite_code"];
  if (typeof inviteFromQuery === "string") {
    data.value.invite_code = inviteFromQuery;
    recordInviteHit(inviteFromQuery);
  }
...
watch(
  ...
  (code) => {
    ...
    if (typeof newCode === "string") {
      data.value.invite_code = newCode;
      recordInviteHit(newCode);
    } else if (!newCode) {
      data.value.invite_code = "";
    }
  }
);
