<template>
  <div class="container media-page">
    <el-tabs v-model="activeName" @tab-change="handleChange">
      ...
      <el-tab-pane label="sora视频" name="luma" v-loading="data.luma.loading">
        <div class="handle-box">
                ...
                <template #default="scope">
                  <div class="container">
                     <!-- <div v-if="scope.row.progress === 100"> -->
                      ...
                      <video class="video" :src="replaceImg(scope.row.video_url)" preload="auto" loop="loop" >您的浏览器不支持视频播放</video>
              ...
              <el-table-column prop="progress" label="任务进度">
                       ...
                       <!-- status  -->
                        ...
                        <template #default="scope">
                          ...
                          <span v-else-if="scope.row.status === 'failed'">已失败</span>
  <!-- <el-tag v-else type="danger">已失败</el-tag> -->
                      </template>
...
const fetchLumaData = () => {
  ...
  //httpPost("/api/admin/media/list/sora", d.query)
    .then((res) => {
      if (res.data) {
        d.items = res.data.items;
        d.total = res.data.total;
