      ...
      <el-table :data="items" :row-key="(row) => row.id" table-layout="auto">
        ...
        <el-table-column label="订单状态">
          <template #default="scope">
            ...
            <el-tag type="info" v-else-if="scope.row.status === 1">其他</el-tag>
            <el-tag type="success" v-else-if="scope.row.status === 2">支付成功</el-tag>
            <el-tag type="danger" v-else-if="scope.row.status === 3">已过期</el-tag>
            <span v-else>未知</span>
          </template>
        </el-table-column>
