      ...
      <div class="mj-box">
        ...
        <div class="mj-params" :style="{ height: paramBoxHeight + 'px' }">
          <el-form :model="params" label-width="80px" label-position="left">
            ...
            <div class="param-line pt">
              <span>画质选择：</span>
              <el-tooltip content="NB: 标准质量模型 <br/>NB2: 高质量模型，消耗更多算力" raw-content placement="right">
                <el-icon>
                  <InfoFilled />
                </el-icon>
              </el-tooltip>
            ...
            <el-form>
              ...
              <el-row class="text-info">
                <el-text type="primary"
                  ...
                  &nbsp;&nbsp; nb2模型消耗<el-text type="warning">{{ mj2Power }}算力;</el-text>
...
const models = [
  { text: "高清质量", value: "nano-banana-2-4k", img: "/images/mj/mj-v5.2.png" },
  { text: "标清质", value: "nano-banana-2-2k", img: "/images/mj/mj-v5.2.png" },
  { text: "标质", value: "nano-banana", img: "/images/mj/mj-v6.png" },
];
      ...
      for (let i = 0; i < jobs.length; i++) {
        if (jobs[i].progress === 101) {
          ...
          if (jobs[i].type === "image") {
            // 根据任务信息判断使用的模型，返还对应的算力
            let refundPower = mjPower.value;
            try {
              if (jobs[i].task_info) {
                const taskInfo = typeof jobs[i].task_info === 'string' 
                  ? JSON.parse(jobs[i].task_info) 
                  : jobs[i].task_info;
                if (taskInfo.full_prompt) {
                  ...
                  if (fullPromptObj.model === "nano-banana-2-4k") {
                    refundPower = mj2Power.value;
                  }
                }
              }
            } catch (e) {
...
const generate = () => {
  ...
  if (activeName.value === "img2img" && imgList.value.length === 0) {
    return ElMessage.error("请先上传参考图片");
  }
  params.value.session_id = getSessionId();
  params.value.img_arr = imgList.value;
  // 根据选择的模型计算算力消耗
  const powerCost = params.value.model === "nano-banana-2-4k" ? mj2Power.value : mjPower.value;
  httpPost("/api/mj/image", params.value)
    .then(() => {
      ElMessage.success("绘画任务推送成功，请耐心等待任务执行...");
      power.value -= powerCost;
      fetchRunningJobs();
    })
    .catch((e) => {
